...
$where                                  
  ...
  array("valor > ?":5, "total < ?":2) // usar ? por segurança, o zend veirifica se não é sql-inject
$order
...
```
```php
$user = Application_Model_UserManager::get(5); 
echo $user['name'];
// exibe o nome do usuario com id 5
```
```php
$user = Application_Model_UserManager::get(array("email = ?" => "teste@teste.com"));
